  ...
  if (!isConfigured()) {
    container.innerHTML = `
      <div class="blog-state" style="grid-column:1/-1; text-align:left; max-width:640px; margin:0 auto;">
        O blog ainda não está conectado ao GitHub.
      </div>`;
    return;
  }
...
async function renderSinglePost() {
  ...
  if (!isConfigured()) {
    container.innerHTML = `<div class="blog-state" style="text-align:left;">O blog ainda não está conectado ao GitHub.</div>`;
    return;
  }
  ...
  try {
    ...
    container.innerHTML = `
      ...
      ${post.cover ? `<div class="post-cover"><img src="${rawUrl(post.cover.replace(/^\//,''))}"></div>` : ''}
